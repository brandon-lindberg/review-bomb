...
BASELINE_PATH = Path("tmp/gpt54-batch200-live-vs-llm-after-native-post-targeted-v14.jsonl")
OUTPUT_JSONL = Path("tmp/gpt54-batch200-live-vs-llm-after-native-post-targeted-v15.jsonl")
OUTPUT_MD = Path("tmp/gpt54-batch200-live-vs-llm-after-native-post-targeted-v15.md")
...
def build_markdown(rows: list[dict[str, object]]) -> str:
    ...
    for row in rows:
        ...
        if row.get("missing_must_include_titles"):
            lines.append(
                f"- missing must-include: {', '.join(row['missing_must_include_titles'])}"
            )
        lines.extend(
            [
...
def main() -> None:
    ...
    for row in baseline_rows:
        ...
        llm_titles = list(row.get("llm_titles") or [])
        ...
        baseline_overlap_count = int(row.get("baseline_overlap_count") or 0)
